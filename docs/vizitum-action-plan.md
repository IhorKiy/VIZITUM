...
- [!] Rerun expanded staging product smoke with `docs/runbooks/expanded-staging-product-smoke.md` after self-serve Field, Admin import and Manager actions deploy. 2026-07-02 recheck passed Field, Admin import and Manager flows; browser audio/file-upload verification still needs a real mic/file-picker browser.
...
- [!] Rerun expanded staging product smoke with `docs/runbooks/expanded-staging-product-smoke.md` for field visit creation, browser voice recording, import validation/confirm, manager CSV export and manager task assignment. Field, Admin import and Manager flows passed on staging; repeat browser voice/audio fallback checks.
