...
- [x] ~~Advance provisioning jobs beyond `queued`/`tenant_created` (`provision` worker task moves tenant `draft`→`ready`, job `queued`→`succeeded`).~~ **Superseded — no `provision` worker task exists.** `parseWorkerTask` (`src/worker.ts`) accepts only `cleanup` and `purge`. Provisioning is legacy: tenants are now created straight into `pilot` and get no `PlatformProvisioningJob` row at all, so there is nothing for a worker to advance. Existing rows are kept for history and stay readable through `GET /platform/tenants/:tenantId`, which is what [`data-model.md`](reference/data-model.md) records. Left ticked and struck through rather than deleted: the step did ship once, and the useful fact for the next reader is that it was removed, not that it never happened (audit F19).
...
- [x] **[audit F6, S3]** (PR #255) **Decided: "destructive" means irreversible — archive stays ungated.** Purge is gated because it ends in data being gone; archive is reversible by `unarchiveTenant`, and a confirmation nobody can act on quickly stops being read. What that accepts, stated plainly: a stolen platform-owner session can archive every tenant in sequence and take the customer base offline unchallenged, with recovery one ungated call per tenant. The reasoning was already in `security-remediation-plan.md` item 1.3's own body; what was generic was its summary row, now corrected, and `archiveTenant` now carries the decision at the code. Original finding: Decide, and write down, whether archiving a tenant needs the second factor purging it requires. `assertFreshSecondFactor` (`platform.service.ts:714`) has exactly one caller — `requestTenantPurge`. `archiveTenant` takes no `mfaCode`, yet it stops the tenant serving requests and revokes every user's session, so a stolen platform-owner session can take the whole customer base offline unchallenged. Archive is reversible and the precondition is a compromised owner session, so this is availability rather than data loss — but `docs/security-remediation-plan.md` records 1.3 as covering "destructive tenant operations" generically, which reads as broader than what shipped. See `docs/plans/full-project-audit-plan.md` finding F6.
...
- [~] **[audit F14, S3]** (PR #258 — first site + the shared map) Render API errors through a code map, not the raw `message`. **The convention is agreed and shipped**: `apps/web/lib/api-error.ts` maps code → key under `common.apiError`, following `lib/login-error.ts`, and an unrecognized code renders the fallback rather than the backend's English. Converted: the field report's refused confirm (`field-visit-report-form.tsx`), the highest-stakes of the set — plus `report-send-outcome.ts`, which now carries the *code* rather than the message, and the two outbox flushes that stored the English prose as an on-device diagnostic. **Still open: 40 `.message` renders** — field 13, admin 11, manager 10, components 3, operations 2, platform 1 — to convert per zone, each adding its codes to the one map. Note the count is higher than the audit's 25 because this sweep also counts `*Outcome.message`, and because the screens have grown since. **F21 depends on this map** and stays open. 25 sites across `admin/*`, `field/*`, `manager/*` and `operations` render `someResult.message` verbatim — the backend's English — directly beneath a next-intl-translated heading, so a Ukrainian tenant reads "Authentication is required." under a Ukrainian title. `web:i18n:check` cannot catch this (it scans for Cyrillic literals; these are English runtime values). `ApiResult` already carries `code`, and `apps/web/lib/login-error.ts` is the pattern to copy — map code → translated key, with a shared fallback. **Start with `field-visit-report-form.tsx:1320`**: it handles a *refused* confirm with `setError(sendOutcome.message || t("saveFailedError"))`, so the translated string is already there and is used only when the English one is empty — the highest-stakes message in the product, on a phone, one `||` from correct. See `docs/plans/full-project-audit-plan.md` finding F14.
...
- [x] Import failure cannot partially corrupt applied data — executed, not read: `tests/import-rollback.test.ts` runs a real import against a real database whose apply fails partway (an external code taken between validation and confirm) and asserts the auto-created chain, category and locations are all absent afterwards, and that the job is still `validated` rather than `applied`. Verified to fail when the wrapping transaction is removed. Until audit F24 this tick rested on a reading of the code.
