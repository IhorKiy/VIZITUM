...
- імпорт точок, представників і продуктів/SKU або позначення продуктів/SKU як not applicable для пілоту;
...
- Якщо користувач має кілька ролей, інтерфейс має показувати role switcher; після login за замовчуванням відкривається останній обраний робочий режим, якщо він досі доступний для користувача.
...
- імпортувати продукти/SKU або позначити їх як not applicable для пілоту;
...
MVP реалізує просту модель маршрутів:
...
`overdue` не є ручним статусом задачі. Це derived flag, який система рахує від `due date` для задач зі статусом `open` або `in_progress`.
...
| Створювати власні візити | Ні | Тільки якщо користувач також має роль Field Representative | Так | Ні |
...
- Team Manager у `Team` створює, редагує і призначає прості route/daily plans паралельно з самостійним плануванням Field Representative у межах призначених йому локацій, а також створює задачі і переглядає точки; власні польові візити Team Manager створює тільки через додаткову роль Field Representative або окремий manager-on-behalf flow, якщо він буде явно доданий; редагування master data по точках лишається Company Admin або окремим дозволом.
...
- можна імпортувати точки, користувачів і продукти/SKU або позначити продукти/SKU як not applicable для пілоту;
