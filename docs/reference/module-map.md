...
- `src/common/` — `request-id.middleware.ts`, `access-log.middleware.ts`, `api-error.filter.ts` (global error envelope), `json-logger.service.ts`, `sentry.service.ts`, `normalize.ts` (shared `normalizeEmail`), `cookie-token.ts` (cookie name-parameterized read/write/clear, used by both tenant and platform session cookies), `session-lifecycle.ts` (shared token issuance + TTL/`revokedAt` activity check, used by both tenant and platform sessions).
