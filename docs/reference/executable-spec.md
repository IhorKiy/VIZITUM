...
| `tests/users-service.test.ts`                   | Tenant-scoped user lifecycle behavior: invites, role assignment and status changes; a user can never be left with zero roles; admin-management actions (status/role changes on a `company_admin` target) run under the `admins.manage`/`admins.invite` gate and the anti-lockout check runs inside a serializable transaction to close a concurrent-request race. See `tests/tenant-superadmin-*.test.ts` for the superadmin-era admin-lockout/limit/protection rules that superseded the old unconditional "last company_admin" rule. |
| `tests/tenant-superadmin-admin-limit.test.ts`   | The tenant's `adminLimit` (`PlatformTenant.adminLimit`, default 2) caps active `company_admin` users; enforced on invite, role-assign and reactivate (409 `TENANT_ADMIN_LIMIT`), and re-checked on invite acceptance to close the "multiple pending invites" gap. |
...
| `tests/tenant-superadmin-replacement.test.ts`   | Accepting a new superadmin invite auto-suspends the previously active superadmin in the same transaction (exactly one active superadmin afterward); `PlatformService.promoteToSuperadmin` swaps an active Company Admin's role in place and demotes any prior active superadmin the same way. |
