...
| Test                                            | Contract                                                                                                                                            |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tests/auth-tenant-isolation.test.ts`           | Tenant-owned access is scoped by request context, not client-provided tenant identifiers. Cross-tenant data must not leak. Also pins the platform bearer token's scope (operations-read only, never tenant-manage) and that `PermissionGuard` bumps `lastSeenAt` on every authenticated platform-session request, not just `/platform/auth/me`. |
