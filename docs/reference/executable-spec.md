...
| Test                                            | Contract                                                                                                                                            |
| ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tests/auth-tenant-isolation.test.ts`           | Tenant-owned access is scoped by request context, not client-provided tenant identifiers. Cross-tenant data must not leak, and invite acceptance with a page tenant slug must match the invite tenant. |
...
| `tests/platform-tenant-creation.test.ts`        | Platform tenant creation validates name/slug/segment template, rejects duplicate slugs, and provisions a new tenant as `draft` with a `queued` provisioning job; list/get expose the registry. Tenant routes require a `platform_owner` session. |
| `tests/platform-provisioning.test.ts`           | The `provision` worker advances `queued` provisioning jobs: a `draft`/`provisioning` tenant moves to `ready` and the job to `succeeded`; an already-active tenant is not dragged back; a tenant with no seeded capabilities fails the job; no queued jobs is a no-op. |
| `tests/platform-tenant-management.test.ts`      | Tenant update validates each editable field, refuses to set `archived` via update (that is the archive action's job), rejects empty payloads and edits to an already-archived tenant, and records `tenant.updated`; archive stamps `archivedAt`, records `tenant.archived`, and is idempotent. |
