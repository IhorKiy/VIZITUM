...
| `PLATFORM_OPERATIONS_TOKEN_SHA256` | recommended | SHA-256 hash of the platform operations bearer token (preferred over plaintext). |
| `PLATFORM_OPERATIONS_TOKEN` | fallback | Plaintext operations token; used only when the SHA256 var is unset. |
...
| Variable | Required | Purpose |
| --- | --- | --- |
| `WORKER_TASK` | no | Task selector; only `cleanup` is supported (default). |
