...
- `location_assortment.manage` vs `location_potential.manage_own`: reads stay one shared permission, but the two tables' **writes** are deliberately separate, because they answer to different owners. The assortment is the standard an outlet must carry, set by the `team_manager` who owns coverage: `PUT`/`DELETE /locations/:locationId/assortment[/:productId]` require `location_assortment.manage` (`@RequirePermissions`), a plain tenant-wide check with no ownership tier — `assertCanManageAssortment` (`src/modules/location-insights/location-insights-access.ts`) 403s `LOCATION_ASSORTMENT_SCOPE_FORBIDDEN` otherwise. The potential is the estimate the assigned representative keeps: `PUT`/`DELETE /locations/:locationId/potential[/:productCategoryId]` require `location_potential.manage_own`, and `assertCanManagePotential` additionally demands an **active `LocationAssignment`** for that location — a live query per request, not a column stamped on the row (403 `LOCATION_POTENTIAL_SCOPE_FORBIDDEN`). A field representative therefore **reads** the assortment but cannot change it, which is what makes the visit report's shelf-check chips (the location's `shouldBeListed` rows) a rule set elsewhere rather than one the rep can edit away. `tenant_superadmin` also holds `location_assortment.manage`, with no screen behind it: a fresh tenant has no manager yet, and someone must be able to repair the data. `company_admin` deliberately holds neither write — the admin zone reviews potential and assortment, it does not author them. Each list response still exposes a computed `canManage: boolean` (from the matching non-throwing helper) so the frontend can hide edit affordances the caller can't use.
