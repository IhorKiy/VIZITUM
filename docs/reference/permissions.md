...
- The `visits.*_own` / `tasks.*_own` permissions are scoped the same way inside their services for reads and updates: the caller's own-scope permission is only honored when the row's owning column (`representativeUserId` / `assignedToUserId`) already equals `context.userId`. Task **creation** is different, since a fresh task has no existing owner to check against — `TasksService.createTask`'s `assertCanAssignOnCreate` instead gates the `assignedToUserId` the caller is allowed to set: a caller holding `tasks.read_team` or `tasks.update_team` (a manager) may assign to anyone or leave a task unassigned; a caller with neither (an own-scope-only `tasks.create` holder — normally a `field_representative`) may only set `assignedToUserId` to themselves and may not omit it, since an unassigned or other-assigned task would be invisible to them on every later own-scope list/update call. Either violation 403s `TASK_SCOPE_FORBIDDEN`, the same code `assertCanUpdateTask` uses for own-scope update violations.
