...
| ----------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /platform/tenants`                         | all: `platform.tenants.read`   | —                                                                                                                              | `PlatformTenant[]`, most recent first, each with `metrics` counts for Company Admins, Team Managers, Field Representatives, visits, products and locations                                                                                                                                                                        |
...
| `GET /platform/tenants/:tenantId/users`         | all: `platform.tenants.read`   | —                                                                                                                              | paginated tenant `User` list for the selected tenant; 404 `TENANT_NOT_FOUND`, 409 `TENANT_ARCHIVED`                                                                                                                                                                 |
| `POST /platform/tenants/:tenantId/users/invite` | all: `platform.tenants.manage` | `{ email, roleCodes: ["company_admin"] }`                                                                                      | `{ id, email, roleCodes, status, expiresAt, token }`; creates a Company Admin invite from platform context, records `tenant.user_invited`; 400 `INVITE_INVALID`/`PLATFORM_INVITE_ROLE_INVALID`, 404 `TENANT_NOT_FOUND`, 409 `TENANT_ARCHIVED`/`USER_ALREADY_EXISTS` |
| `PATCH /platform/tenants/:tenantId/users/:userId/status` | all: `platform.tenants.manage` | `{ status: "active" \| "suspended" }`                                                                                          | updated tenant `User`; only Company Admin users can be managed here, suspension revokes that user's active tenant sessions, and events are recorded as `tenant.admin_suspended`/`tenant.admin_reactivated`; 400 `PLATFORM_ADMIN_STATUS_INVALID`/`PLATFORM_ADMIN_ROLE_REQUIRED`, 404 `TENANT_NOT_FOUND`/`USER_NOT_FOUND`, 409 `TENANT_ARCHIVED`/`TENANT_LAST_ADMIN` |
| `POST /platform/tenants`                        | all: `platform.tenants.manage` | `{ name, slug, segmentTemplate, country?, timezone?, language?, primaryDomain? }`                                              | `{ tenant }` (tenant created with `status: "pilot"` — live immediately, no provisioning delay); errors: 400 `TENANT_INVALID` (missing/invalid fields), 409 `TENANT_SLUG_ALREADY_EXISTS`                                                                            |
| `PATCH /platform/tenants/:tenantId`             | all: `platform.tenants.manage` | `{ name?, timezone?, language?, primaryDomain?, status? }` — `status` doubles as the plan tier, so this is also how plan changes: assignable values are `pilot`/`team`/`business`/`suspended`; `archived` goes through the archive action, and `draft`/`provisioning`/`ready`/`active` are retired (nothing advances a tenant through them anymore) | updated `PlatformTenant`; errors: 400 `TENANT_UPDATE_INVALID`/`TENANT_UPDATE_EMPTY`, 404 `TENANT_NOT_FOUND`, 409 `TENANT_ARCHIVED`                                                                                                                                  |
...
73 endpoints across 16 controllers (auth 5, health 2, operations 1, platform auth 3, platform 5, platform tenant users 2, pilot review 2, visits 11, tasks 3, locations 11, products 4, routes 6, imports 6, admin users 7, admin settings 2, storage 3).
