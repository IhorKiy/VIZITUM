...
### Tier 5 — `imports` (1 route) — **done**

`POST /imports/jobs/validate`, gated through `CreateImportValidationJobDto` and pinned by `tests/imports-dto-validation.test.ts`. The design note this tier asked for is written: [`imports-dto-migration-note.md`](imports-dto-migration-note.md). It reaches the same conclusion as the `visits` one — gate the envelope, leave the blob opaque — by an entirely different argument, since `csvText` is a *scalar* and there is nothing for a whitelist to walk (the offline-replay reasoning that settled `confirmedData` has no counterpart, as this prompt predicted). Its Q2 is the part that generalizes: the note, the DTO's own docblock and `api-reference.md` all say plainly what the gate does **not** buy, because "this route is now gated by a class-validator DTO" would otherwise read as though the import path had been validated, when everything that defends it (`assertApprovedHeader`, the per-column caps, the formula guard, preview-then-confirm) sits behind the pipe untouched.
