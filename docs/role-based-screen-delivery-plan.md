...
| Team Manager          | `/:tenantSlug/manager/visits` | Live tenant visit table with status filters, filter context and filtered-empty recovery actions.                | Live   |
| Team Manager          | `/:tenantSlug/manager/tasks`  | Live team task table with status/priority filters, task status updates and filtered-empty recovery actions.     | Live   |
...
| Screen                  | MVP intent                                                  | Current state                                                                                                                                                      | Next delivery step                                                                               | Priority |
| ----------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ | -------- |
| Onboarding checklist    | Guide customer setup before pilot activation.               | Live and staging-smoked at `/:tenantSlug/admin/setup` with live setup progress for users, roles, locations, products/SKUs, import templates and pilot readiness.   | Add deeper tenant settings and applied import history once those screens exist.                  | P0       |
...
| Users                   | Invite, view, deactivate users and manage roles.            | Live and staging-smoked at `/:tenantSlug/admin/users` with user list, invite creation, suspend/reactivate and add/remove role controls with pending submit states. | Add pending invite history once invite listing exists.                                           | P0       |
...
| Visits          | Review all tenant visits and AI/manual summaries.             | Live and staging-smoked at `/:tenantSlug/manager/visits` with status filters, counters, tenant visit table and clearer empty filtered states.             | Add report detail drilldown when report detail UI exists.                   | P0       |
| Tasks           | View/create/update team tasks.                                | Live and staging-smoked at `/:tenantSlug/manager/tasks` with status/priority filters, counters, status update controls and clearer empty filtered states. | Add richer task board/grouping after pilot-critical table view is verified. | P0       |
