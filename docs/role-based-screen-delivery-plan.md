...
| Platform / Operations | `/platform/tenants`                    | Platform-owner tenant registry with create, status update, archive, Company Admin listing, tenant-scoped Company Admin invite creation and Company Admin suspend/reactivate actions. | Live   |
...
| -------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | -------- |
| Tenant list                | See all tenant workspaces and status.                             | Live at `/platform/tenants` for platform owners.                                              | Add richer tenant detail only when production operations console needs it.                                      | Done     |
| Create tenant              | Create pilot tenant, select template, invite Company Admin.       | Live at `/platform/tenants`: create tenant, update status, archive, invite Company Admins and suspend/reactivate Company Admins. | Add resend/cancel invite controls only if platform onboarding needs them outside tenant admin.                  | Done/P1  |
...
| Visits          | Review all tenant visits and AI/manual summaries.             | Live and staging-smoked at `/:tenantSlug/manager/visits` with route, status, representative and started date filters, counters and tenant visit table; report detail lives at `/:tenantSlug/manager/visits/:visitId`. | Add richer template-specific formatting only if pilot managers need it.                                | P0       |
...
| Tasks                 | See and update own tasks.                           | Field page own task panel is staging-smoked with priority/due details, real location names and status update controls.     | Add richer task detail only if pilot representatives need it after the table view is verified.  | P0       |
...
7. Platform create tenant and first tenant admin invite. Initial version implemented at `/platform/tenants`; richer tenant detail remains optional.
