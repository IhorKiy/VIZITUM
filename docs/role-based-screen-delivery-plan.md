...
| Users                   | Invite, view, deactivate users and manage roles.            | Live and staging-smoked at `/:tenantSlug/admin/users` with user list, invite creation, suspend/reactivate and add/remove role controls with pending submit states. Local P1 update adds invite history, expiry visibility and fresh-token resend. | Re-smoke invite history and resend after the next deploy.                                        | P0       |
...
| Imports                 | Download templates, validate, review issues and confirm.    | Implemented and staging-smoked. Local P1 update adds tenant-scoped import history with row counts, statuses, uploader/confirmer and applied output summary.        | Re-smoke import history against staging jobs after the next deploy.                              | P1       |
...
| Visit form            | Start visit and add text/audio note.                | Implemented and staging-smoked. Local pilot-candidate polish adds audio retry/manual fallback action links and clearer recorder fallback copy. | Re-smoke audio success/error recovery after the next deploy.                                    | P0       |
...
1. Company Admin users screen: list, invite, role assignment and deactivate. Initial screen implemented; local P1 update adds pending invite history, expiry visibility and fresh-token resend.
...
1. Admin import history with applied row counts and validation failure history. Initial version implemented; staging re-smoke pending after deploy.
