...
- **The gate did its job on 2026-08-04, and the count moved again — to 4 accepted plus 1 fixed.** Two new high advisories landed against dependencies nothing in this repo had touched, turning Backend Checks red on every open PR at once (`main` was green the day before). Both were read: **`fast-uri`** (GHSA-7p8r-x3mc-p8w7, host confusion via a backslash authority introducer) is reached only through `prisma`, a devDependency, via `@prisma/dev` → `@prisma/streams-local` → `ajv`, so it never enters the deployed API at all — but `ajv` asks for `^3.0.1` and the fix shipped in `3.1.5`, so this one was **fixed rather than accepted**: a three-line lockfile bump, no override, no range change. Accepting an advisory that a patch release closes is how the list stops meaning anything. **`postcss`** GHSA-fxqj-rqcc-2cmp is the incomplete-fix follow-up to GHSA-6g55-p6wh-862q already on the list — the same arbitrary file read by the same route (a `sourceMappingURL` in the input, followed when `from` is unset), so the same reach argument holds: build time, over this repo's own CSS, and an attacker who can write those stylesheets is already running our build. It is accepted because there is still nothing to move to — `next@16.2.12` pins postcss to exactly `8.4.31`, not a range, and the advisory is open through `8.5.22`.
