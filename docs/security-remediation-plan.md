...
- **The gate fired on 2026-08-04, exactly as designed, and the two findings split.** Nothing in the tree had changed — `main` was green the day before on the same lockfile — so this was the advisory database moving under a fixed dependency set, which is the case a recorded verdict cannot cover and this check exists to catch.
  - **`fast-uri` [high], GHSA-7p8r-x3mc-p8w7 (host confusion via backslash authority introducer) — fixed, not accepted.** Reached as `@prisma/client` → `prisma` → `@prisma/dev` → `@prisma/streams-local` → `ajv` → `fast-uri`. Worth noting the path runs through `@prisma/client`, a *runtime* dependency, not only the dev CLI — so "it's dev tooling" would have been the wrong reason to wave it through. It needed no reason at all: `ajv` declares `fast-uri: ^3.0.1`, so `npm update fast-uri` moves it 3.1.4 → 3.1.5 (patched) inside the existing range. Three lines of `package-lock.json`, no declared range touched. **Prefer this outcome — the accepted list is for advisories with no reachable fix, not for ones that are merely inconvenient to bump.**
  - **`postcss` [high], GHSA-fxqj-rqcc-2cmp — accepted, making four.** It is the incomplete-fix follow-up to GHSA-6g55-p6wh-862q, already on the list: same package, same arbitrary `.map` read, now also reachable when `from` is unset. Same reach and same immovability — `next` pins `postcss` at the exact version `8.4.31`, not a range, so unlike `fast-uri` no lockfile bump can reach the patched 8.5.23. Re-verified rather than inherited: `apps/web` has no postcss config of its own, and the one place tenant input becomes CSS (`buildSchemeStyle`) emits a runtime `<style>` from the fixed `TENANT_COLOR_SCHEMES` allowlist via `toColorScheme`, which never reaches postcss.
- **How to check this is still true: CI does it now.** `npm run audit:check` (`scripts/audit-check.mjs`, wired into Backend Checks) fails on any high or critical advisory that is not on an explicit accepted list, keyed on advisory id rather than package name — so a *new* postcss finding blocks even though postcss is accepted for the three it has today. `next` is accepted only while its `via` contains nothing but package names; an object there means it carries a finding of its own, which is the exact tripwire this bullet used to describe in prose. A bare `npm audit --audit-level=high` was rejected as the gate: it would be red from the first commit for advisories with no fix, and a permanently red check is one nobody reads. `tests/audit-allowlist.test.ts` pins the accept/block rules.
