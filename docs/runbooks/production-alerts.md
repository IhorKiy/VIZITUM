...
| Sign-in audit trail not being written | Sentry errors with `errorCode=AUTH_AUDIT_WRITE_FAILED`, or structured logs where `message=auth_audit_write_failed` | Warning  | Any occurrence                                                                                | The sign-in trail is best-effort by design, so a failure is swallowed to keep logins working — which means the only symptom is an empty trail that looks like a quiet week. Check the database and the `audit_events`/`platform_operation_events` writes. |
