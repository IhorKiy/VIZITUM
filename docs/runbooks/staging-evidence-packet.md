...
| Product smoke          | Login, tenant lookup, field, imports, manager dashboard and manual report confirmation passed | Staging pass   | Expanded smoke on 2026-07-02 passed login, Field visit/text note/manual report, browser recording/upload, audio file fallback upload, Admin template proxy/validation/confirm, Manager dashboard/export/task and expected Operations permission state. Admin import job `cmr3rwtzh00012bd75jhtliyt` validated 1 row with 0 errors and applied 1 row. |
...
| Expanded self-serve product smoke      | 2026-07-02 smoke passed Field recording/fallback, Admin and Manager core flows              | Product smoke passed with operations-token and production ops gates still tracked separately.                                                                            |
...
3. Choose paid PostgreSQL backup/export/PITR path before production pilot.
4. Run restore drill after selecting a backup-capable PostgreSQL plan/provider.
