...
| Product smoke          | Login, tenant lookup, field, imports, manager dashboard and manual report confirmation passed | Follow-up      | Expanded smoke on 2026-07-02 passed login, Field visit/text note/manual report, Manager dashboard/export/task and expected Operations permission state. Deployed Admin template links failed with `/api/api/...`; browser audio/file upload checks need repeat in a real mic/file-picker browser. |
...
| Expanded self-serve product smoke      | 2026-07-02 smoke found deployed Admin template path and browser automation limitations        | Must repeat before self-serve pilot after deploying the import path fix and rerunning audio/import upload checks.                                                       |
...
6. Deploy the Admin import template download path fix and rerun Admin template, validation and confirm smoke.
