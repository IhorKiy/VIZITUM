...
| Product smoke          | Login, tenant lookup, field, imports, manager dashboard and manual report confirmation passed | Follow-up      | Expanded smoke on 2026-07-02 passed login, Field visit/text note/manual report, browser recording/upload, Admin template proxy/validation/confirm, Manager dashboard/export/task and expected Operations permission state. Admin import job `cmr3rwtzh00012bd75jhtliyt` validated 1 row with 0 errors and applied 1 row. Audio file fallback failed on staging; local MIME alias fix is pending deploy/retest. |
...
| Expanded self-serve product smoke      | 2026-07-02 smoke passed Field recording, Admin and Manager core flows; audio fallback failed | Deploy MIME alias fix and repeat audio file fallback before self-serve pilot.                                                                                            |
...
6. Deploy the audio fallback MIME alias fix and repeat audio file fallback in a browser/device that supports file upload.
