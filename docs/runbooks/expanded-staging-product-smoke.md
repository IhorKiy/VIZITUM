...
| Field audio fallback          | Upload a small supported audio file through the same voice note control.                     | Success notice appears after upload.                                                           | Real-browser check by Ihor on 2026-07-02: selecting an existing audio file returned an upload failure. Local follow-up normalizes browser MIME aliases such as `audio/x-m4a`, `audio/wave` and `audio/mp3`. | Fail on deployed staging |
...
| ---- | ----- | -------- | ----------- | ----- |
| Deploy audio fallback MIME alias fix and repeat file upload fallback | Ihor Kiyanych | Medium | Before self-serve pilot | Browser recording passed; existing audio file upload failed on staging. Local fix normalizes common browser audio MIME aliases before register/upload. |
...
- Operator: Codex with Ihor Kiyanych staging login
- Reviewer:
- Decision: Passed with follow-up. Browser recording and Admin import flows are verified; repeat required for audio file fallback after MIME alias fix deploy and for operations token verification.
