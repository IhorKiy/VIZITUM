...
| Admin import template         | Open Admin imports and download a template.                                                  | CSV template downloads from backend URL.                                                       | Imports page loaded live templates, but deployed card links point to `https://vizitum-api-staging.onrender.com/api/api/imports/templates/*.csv`; local fix changes paths to `/imports/templates/*.csv`. | Fail on deployed staging |
...
| ---- | ----- | -------- | ----------- | ----- |
| Deploy import template download path fix and repeat Admin template/validation/confirm smoke | Ihor Kiyanych | High | Before self-serve pilot | Deployed staging currently emits `/api/api/imports/templates/*.csv`; local code now emits `/imports/templates/*.csv`. |
...
- Operator: Codex with Ihor Kiyanych staging login
- Reviewer:
- Decision: Repeat required after import path fix deploy and manual/real-browser audio/import upload verification.
