...
| Import history needs staging re-smoke                  | Admin imports       | Tenant-scoped history is implemented locally with row counts, status and applied output summary; verify it against retained staging import jobs after deploy.                    |
