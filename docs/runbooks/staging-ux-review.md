...
| Admin setup/users/review     | Pass    | Setup checklist, users/roles screen and pilot review summary render live tenant data without demo fallback; Admin users mutations have pending/disabled submit states. Invite history, expiry visibility and fresh-token resend passed staging re-smoke with `kiyanichenko81@gmail.com` on 2026-07-03. |
| Admin imports page load      | Pass    | Page loads live templates; per-template downloads route through tenant-local Next proxy; one-row users import validated with 0 errors and confirmed with 1 applied row. Tenant-scoped import history passed staging re-smoke with retained applied and validated jobs on 2026-07-03.              |
...
| Empty live lists need clear next actions               | Field/Manager       | Field and Manager now avoid demo-looking fallback for empty live lists; Manager filtered drilldowns show context, route/date/assignee filters and recovery actions.            |
...
1. Repeat platform operations token and operations summary alert check for production after production services are created.
2. Repeat UX review against staging if any new product-surface changes are introduced before pilot.
