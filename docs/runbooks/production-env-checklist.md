...
| `TOTP_ENCRYPTION_KEY` | `openssl rand -base64 32` | Yes | Encrypts the platform owner's TOTP secret at rest. **Set it before deploying the release that introduces it** — the service refuses to start in production without a valid 32-byte key, so setting it afterwards means a failed boot in between. Keep it with the database credentials: losing it makes existing secrets undecryptable, and recovery is the owner signing in with a recovery code and re-enrolling. After the first deploy, run `npm run encrypt:totp-secrets` once to encrypt the secret already stored; without it that row stays in the clear until the owner next signs in. |
