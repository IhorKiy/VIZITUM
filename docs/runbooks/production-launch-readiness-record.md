...
| Smoke checks       | Login, tenant lookup, field, imports, manager dashboard, manual report confirmation pass                         | Follow-up required     | Expanded staging smoke on 2026-07-02 passed login, Field visit/text note/manual report, Admin template proxy/validation/confirm, Manager dashboard/export/task and expected Operations permission state. Audio recording/file upload were not verified in the Codex browser environment. |
...
| Expanded product smoke still needs real-browser audio verification.                           | Medium   | Ihor Kiyanych | Repeat audio recording/upload checks in a real browser/device, then repeat the accepted smoke path against production before pilot launch. | Yes             |
...
- Rerun expanded staging product smoke audio recording/upload verification in a real browser/device.
