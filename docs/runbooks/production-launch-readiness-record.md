...
| Smoke checks       | Login, tenant lookup, field, imports, manager dashboard, manual report confirmation pass                         | Follow-up required     | Expanded staging smoke on 2026-07-02 passed login, Field visit/text note/manual report, Manager dashboard/export/task and expected Operations permission state. It failed self-serve readiness because deployed Admin template links used `/api/api/...`; audio recording/file upload were not verified in the Codex browser environment. |
...
| Expanded product smoke found follow-up issues on 2026-07-02.                                  | High     | Ihor Kiyanych | Deploy the local Admin template path fix, repeat Admin validation/confirm and repeat audio recording/upload checks in a real browser, then repeat the accepted smoke path against production before pilot launch. | Yes             |
...
- Deploy the Admin import template path fix and rerun expanded staging product smoke with Admin validation/confirm and audio upload verification.
