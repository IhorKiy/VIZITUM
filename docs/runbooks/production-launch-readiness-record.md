...
| Smoke checks       | Login, tenant lookup, field, imports, manager dashboard, manual report confirmation pass                         | Follow-up required     | Expanded staging smoke on 2026-07-02 passed login, Field visit/text note/browser recording/manual report, Admin template proxy/validation/confirm, Manager dashboard/export/task and expected Operations permission state. Audio file fallback failed on staging; local MIME alias fix is pending deploy/retest. |
...
| Audio file fallback failed during expanded product smoke.                                     | Medium   | Ihor Kiyanych | Deploy the MIME alias fix, repeat audio fallback in a real browser/device, then repeat the accepted smoke path against production before pilot launch. | Yes             |
...
- Deploy the audio fallback MIME alias fix and rerun expanded staging product smoke audio file fallback verification.
