...
| Alerts             | `docs/runbooks/production-alerts.md` readiness verification and `npm run alerts:check` completed                 | Staging pass           | UptimeRobot readiness monitor created with status Up. `npm run alerts:check` passed for API readiness and web URL; operations summary check skipped because operator token env is not configured.                                                   |
...
| Operations summary | `/api/operations/summary` returns aggregate counters and the web operations page renders for an operator account | Pending                | Token path exists. Not verified until `PLATFORM_OPERATIONS_TOKEN_SHA256` or staging `PLATFORM_OPERATIONS_TOKEN` is configured and `alerts:check` is rerun with `OPERATIONS_SUMMARY_URL`.                                                            |
| Smoke checks       | Login, tenant lookup, field, imports, manager dashboard, manual report confirmation pass                         | Follow-up required     | Expanded staging smoke on 2026-07-02 passed login, Field visit/text note/manual report, Manager dashboard/export/task and expected Operations permission state. It failed self-serve readiness because Admin template browser download still needs the local authenticated Next proxy fix; audio recording/file upload were not verified in the Codex browser environment. |
...
| Operations summary endpoint is not verified.                                                     | Medium   | Ihor Kiyanych | Configure platform operator token env and run `alerts:check` with `OPERATIONS_SUMMARY_URL` and `OPERATIONS_SUMMARY_BEARER_TOKEN`.                                             | No              |
...
| Expanded product smoke found follow-up issues on 2026-07-02.                                  | High     | Ihor Kiyanych | Deploy the local Admin template download proxy fix, repeat Admin validation/confirm and repeat audio recording/upload checks in a real browser, then repeat the accepted smoke path against production before pilot launch. | Yes             |
...
- Deploy the Admin import template download proxy fix and rerun expanded staging product smoke with Admin validation/confirm and audio upload verification.
