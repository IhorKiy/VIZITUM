...
- Field offline resilience: drafts, unsent recordings/photos and confirmed reports survive a reload, a killed tab and a dead phone, and send themselves once signal returns; starting a visit no longer needs a connection at the moment of the tap. The whole series is merged, including the visit-start frontend and a route snapshot behind a minimal service worker. A partial real-phone pass has run (iOS 18.7.9, 2026-08-03, recorded scenario by scenario in `docs/runbooks/field-offline-iphone-test.md`): everything built to keep work *safe* held, and the storage layer lost nothing across five scenarios. What did not hold is reachability - no navigation completes without a network on iOS, so start/continue/save land on the offline shell, and a cold launch gets Safari's own error page (withdrawn as unfixable from this codebase rather than treated as a bug). Read `docs/plans/offline-field-drafts-plan-prompt.md` before touching any of it: it carries the current status and the known gaps.
