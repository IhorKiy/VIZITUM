...
//
// A bare `npm audit --audit-level=high` cannot be the gate here, even though
// the accepted list happens to be empty today. Advisories with no fix do
// appear — four sat here from 2026-08-02 until next@16.3.0 finally shipped a
// postcss and sharp that closed them — and while one is open, `npm audit` is
// red on every commit, which is how a check stops being read. What matters is
// not "are there advisories" but "are there advisories nobody has looked at",
// so every accepted one is listed below with its reason, and anything else
// fails.
//
...
// found to have next year; accepting an id accepts exactly the thing that was
// read and judged.
//
...
// Empty as of 2026-08-04: `npm audit` reports nothing high or critical. The
// four entries that lived here — three postcss, one sharp, all reached only
// through next — were removed once next@16.3.0 brought postcss 8.5.23 and
// sharp 0.35.3, since an entry that no longer matches anything stops being a
// judgement and becomes furniture (main() names them on the way past for
// exactly that reason). See docs/security-remediation-plan.md item 3.7.
//
// Shape of an entry, for whoever adds the next one:
//
//   { id: 1117015, package: "postcss", reference: "GHSA-qx2v-qp2m-jg93",
//     reason: "Why this cannot reach this app, in a sentence or two." }
//
// `id` is npm's numeric advisory source, which is what the report is matched
// on; `reference` is the GHSA slug, which is what a human recognises.
...
// The lists are parameters with defaults rather than closed-over constants so
// the test can pin the rules against fixtures. It used to reach for whatever
// ids happened to be accepted that week, which meant the suite could only pass
// while the list was non-empty — the day the advisories were actually fixed,
// three tests failed for having nothing left to point at. The behaviour under
// test is the matching, not the contents.
export function evaluateAudit(
  report,
  acceptedAdvisories = ACCEPTED_ADVISORIES,
  acceptedParents = ACCEPTED_PARENTS,
) {
  ...
  const stale = acceptedAdvisories.filter(
